...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const {result, error, imageUrl} = await getData("testUser", "9"); // Assuming getData fetches image data including Data
  ...
  return(  
    <div className={styles.container}>
      ...
      {imageData && (
        <Image src={imageData} alt="loading..." width={600} height={900} />
      )}
    </div>
  );
}
